...
// >>>
void TSShapeInstance::reSkin( String newBaseName, String oldBaseName, String refPath )
{
   // >>>
   // This function assumed, that both the old and the new base name textures reside in the same
   // directory as the shape file. This can now be overridden by specifying refPath to give a
   // reference where the textures reside.
   // <<<
   ...
   String resourcePath = shapePath.getPath();
