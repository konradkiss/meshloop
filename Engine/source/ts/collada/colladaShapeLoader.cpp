...
// >>>
ConsoleFunction( colladaToDTS, bool, 2, 2, "( path ) - Create a cached dts version of a collada shape." )
{
   TORQUE_UNUSED(argc);
   TSShape* retShape = loadColladaShape( argv[1] );
   bool ret = retShape != NULL;
   delete retShape;
   return ret;
}
// <<<
