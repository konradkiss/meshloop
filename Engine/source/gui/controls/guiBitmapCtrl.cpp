...
void GuiBitmapCtrl::initPersistFields()
{
   addGroup("GuiBitmapCtrl");		
      ...
      // >>>
      addField("bgColor", TypeColorI,     Offset(mBgColor,       GuiBitmapCtrl));
      // <<<
...
// >>>
void GuiBitmapCtrl::setBgColor(ColorI col)
{
   mBgColor = col;
}

ConsoleMethod( GuiBitmapCtrl, setBgColor, void, 6, 6, "int r,g,b,a")
{
   ColorI color(dAtoi(argv[2]), dAtoi(argv[3]), dAtoi(argv[4]), dAtoi(argv[5]));
