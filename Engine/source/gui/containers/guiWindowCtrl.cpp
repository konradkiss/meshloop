...
void GuiWindowCtrl::initPersistFields()
{
   ...
   // >>>
   addField("resizeTop",         TypeBool,         Offset(mResizeTop, GuiWindowCtrl));
   addField("resizeBottom",      TypeBool,         Offset(mResizeBottom, GuiWindowCtrl));
   // <<<
...
// >>>
ConsoleMethod( GuiWindowCtrl, forceResize, void, 2, 2, "() - Force a resize to the same dimensions." )
{
   TORQUE_UNUSED(argc); TORQUE_UNUSED(argv);
   object->resize(object->getPosition(), object->getExtent());
}
// <<<
...
void GuiWindowCtrl::onMouseDown(const GuiEvent &event)
{
   ...
   // >>>
   if( mResizeHeight && hitEdges & edgeBottom && mResizeBottom )
      mResizeEdge |= edgeBottom;
   else if( mResizeHeight && hitEdges & edgeTop && mResizeTop )
      mResizeEdge |= edgeTop;
   else
      mMouseResizeHeight = false;
   // <<<
...
void GuiWindowCtrl::onMouseDragged(const GuiEvent &event)
{
   ...
   else
   {
      if( ( !mMouseResizeHeight && !mMouseResizeWidth ) || !parent )
		...
		mResizeWindow = true;
      // >>>
      if( mResizeEdge & edgeBottom && mResizeBottom)
		{
         newExtent.y = getMin(parent->getHeight(), mOrigBounds.extent.y + deltaMousePosition.y);
		}
      else if ( mResizeEdge & edgeTop && mResizeTop)
      {
         newPosition.y = mOrigBounds.point.y + deltaMousePosition.y;
         newExtent.y = getMin(parent->getHeight(), mOrigBounds.extent.y - deltaMousePosition.y);
         // let's check if this would move the whole window..
         // if it can't move, then don't make any adjustments to it
         if (newExtent.len()<mMinExtent.len() && !mCanMove) {
            mResizeWindow = false;
...
void GuiWindowCtrl::getCursor(GuiCursor *&cursor, bool &showCursor, const GuiEvent &lastGuiEvent)
{
   ...
   // >>>
   if( hitEdges & edgeBottom && hitEdges & edgeLeft && mResizeHeight && mResizeBottom )
      desiredCursor = PlatformCursorController::curResizeNESW;
   else if( hitEdges & edgeBottom && hitEdges & edgeRight && mResizeHeight && mResizeBottom )
      desiredCursor = PlatformCursorController::curResizeNWSE;
   else if( hitEdges & edgeBottom && mResizeHeight && mResizeBottom )
      desiredCursor = PlatformCursorController::curResizeHorz;
   else if( hitEdges & edgeTop && hitEdges & edgeLeft && mResizeHeight && mResizeTop )
      desiredCursor = PlatformCursorController::curResizeNWSE;
   else if( hitEdges & edgeTop && hitEdges & edgeRight && mResizeHeight && mResizeTop )
      desiredCursor = PlatformCursorController::curResizeNESW;
   else if( hitEdges & edgeTop && mResizeHeight && mResizeTop )
      desiredCursor = PlatformCursorController::curResizeHorz;
