                           ...
                           mCanHit( true )
{
   ...
   // >>>
   mMouseEventsTarget = String::EmptyString;
   // <<<
...
void GuiControl::initPersistFields()
{
   ...
   addField("passMouseEventsTo", TypeString,       Offset(mMouseEventsTarget, GuiControl));
...
void GuiControl::removeChildren()
{
   ...
   while (i != begin())
   {
      ...
      if (ctrl->getParent()==this) {
         ctrl->removeObject(ctrl);
      }
   }
}

ConsoleMethod( GuiControl, removeChildren, void, 2, 2, "")
{
...
void GuiControl::onMouseUp(const GuiEvent &event)
{
   ...
   GuiControl *target = dynamic_cast<GuiControl*> (Sim::findObject(mMouseEventsTarget) );
   if ( target ) {
...
bool GuiControl::onMouseWheelUp( const GuiEvent &event )
{
   ...
   GuiControl *target = dynamic_cast<GuiControl*> (Sim::findObject(mMouseEventsTarget) );
   if ( target ) {
      return target->onMouseWheelUp( event );
   }
...
bool GuiControl::onMouseWheelDown( const GuiEvent &event )
{
   ...
   GuiControl *target = dynamic_cast<GuiControl*> (Sim::findObject(mMouseEventsTarget) );
   if ( target ) {
      return target->onMouseWheelDown( event );
   }
...
void GuiControl::onRightMouseDragged(const GuiEvent &event)
{
   ...
   GuiControl *target = dynamic_cast<GuiControl*> (Sim::findObject(mMouseEventsTarget) );
   if ( target ) {
      target->onRightMouseDragged( event );
      return;
   }
