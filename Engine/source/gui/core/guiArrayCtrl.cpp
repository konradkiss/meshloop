...
// >>>
bool GuiArrayCtrl::cellSelected(Point2I cell, bool doubleClick)
{
   ...
   onCellSelected(cell, doubleClick);
	setUpdate();
   return true;
}

void GuiArrayCtrl::onCellSelected(Point2I cell, bool doubleClick)
{
   if (doubleClick) {
  	   Con::executef(this, "onSelectDblClick", Con::getFloatArg(cell.x), Con::getFloatArg(cell.y));
   } else {
...
void GuiArrayCtrl::onMouseDown(const GuiEvent &event)
{
   ...
   if (cell.x >= 0 && cell.x < mSize.x && cell.y >= 0 && cell.y < mSize.y)
   {
      ...
      //select the new cell
      cellSelected(Point2I(cell.x, cell.y), event.mouseClickCount>1);
