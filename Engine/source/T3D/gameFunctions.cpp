...
// >>>
// this function reads in two non-transparent images
// with f0f and 0f0 backgrounds, recomputes alpha and color information
// and outputs the original image with a transparent background
void makeTransparent(const char * b1Path, const char * b2Path, const char * oPath)
{
   Resource<GBitmap> b1 = GBitmap::load( b1Path );
   if(!b1) {
      Con::errorf("makeTransparent - could not load file '%s'!", b1Path);
      return;
   }
   Resource<GBitmap> b2 = GBitmap::load( b2Path );
   if(!b2) {
      Con::errorf("makeTransparent - could not load file '%s'!", b2Path);
      return;
   }
   if (b1->getFormat()!=b2->getFormat() || b1->getWidth()!=b2->getWidth() || b1->getHeight()!=b2->getHeight()) {
      Con::errorf("makeTransparent: format or dimensions mismatch.");
      return;
   }
   ColorI bgcol = ColorI(255, 0, 255, 255);
   GBitmap o = GBitmap(b1->getWidth(), b1->getHeight(), false, GFXFormatR8G8B8A8);
   for (U32 y=0;y<b1->getHeight();y++) {
      for (U32 x=0;x<b2->getWidth();x++) {
         ColorI b1col = ColorI(255, 0, 255, 255);
         ColorI b2col = ColorI(0, 255, 0, 255);
         b1->getColor(x, y, b1col);
         b2->getColor(x, y, b2col);
         U8 alpha = 255-((U8)mAbs(b1col.red-b2col.red));
         ColorI ocol;
         ocol.alpha  = (U8)(alpha);
         ocol.red    = (U8)((F32)bgcol.red - ((F32)(bgcol.red-b1col.red)/(F32)(alpha/255)));
         ocol.green  = (U8)((F32)bgcol.green - ((F32)(bgcol.green-b1col.green)/(F32)(alpha/255)));
         ocol.blue   = (U8)((F32)bgcol.blue - ((F32)(bgcol.blue-b1col.blue)/(F32)(alpha/255)));
         o.setColor(x, y, ocol);
      }
   }
   FileStream fs;
   if ( fs.open( oPath, Torque::FS::File::Write ) )
      o.writeBitmap( "png", fs );
}

ConsoleFunction(makeTransparent, void, 4,4, "(string source1, string source2, string target) - Creates a transparent png from two png-s with known backgrounds.")
{
   TORQUE_UNUSED(argc);
   makeTransparent(argv[1], argv[2], argv[3]);
}
// <<<