...
{
   // >>>
   Con::warnf(ConsoleLogEntry::General, "Attempting to map material %s to %s", materialName.c_str(), textureName.c_str());
   // <<<
   if (getMapEntry(textureName).isNotEmpty())
   {
      if (!textureName.equal("unmapped_mat", String::NoCase))
         // >>>
         Con::warnf(ConsoleLogEntry::General, "Overwriting material %s with %s for %s", getMapEntry(textureName).c_str(), materialName.c_str(), textureName.c_str());
         // <<<
   }
...
// >>>
void MaterialManager::clearMapEntry(const String & textureName)
{
   MaterialMap::Iterator iter = mMaterialMap.find(String::ToLower(textureName));
   if ( iter == mMaterialMap.end() )
      return;
   mMaterialMap.erase(String::ToLower(textureName));
}
// <<<
